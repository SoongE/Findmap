...
const users = {
    ...
    patchFollow: async (req, res) => { // 팔로우 등록, 취소
        ...
        if (!followingIdx) return res.json({success: false, code: 2032, message: "followingIdx를 입력해 주세요."});
        if (followerIdx == followingIdx) return res.json({success: false, code: 2032, message: "본인을 팔로우할 수 없습니다."});
        ...
        try {
            ...
            const [checkFollow] = await userModel.checkFollow(followerIdx,followingIdx);

            if (checkFollow.length < 1) { //follow가 존재하지 않는다면

                const result = await userModel.postFollow(followerIdx,followingIdx);
                return res.json({success: true, code: 1000, message: "팔로우 성공"});

            } else { //follow가 존재한다면

                if (checkFollow[0].status == 'Y'){
                    const result = await userModel.deleteFollow(followerIdx,followingIdx);
                    return res.json({success: true, code: 1000, message: "팔로우 취소 성공"});
                } 
                if (checkFollow[0].status == 'D'){
                    const result = await userModel.reuseFollow(followerIdx,followingIdx);
    ...
    getFollowerList: async (req, res) => {
        ...
        try {
            ...
            // 팔로워가 있는지 확인
            const [checkFollowing] = await userModel.checkFollowing(userIdx);
            if (checkFollowing[0] == undefined){
                return res.json({success: true, code: 3101, message: "팔로워가 존재하지 않습니다."});
            }
            
            const followerRow = await userModel.selectFollowerList(userIdx);
    ...
    getFollowingList: async (req, res) => {
        ...
        try {
            ...
            // 팔로잉이 있는지 확인
            const [checkFollower] = await userModel.checkFollower(userIdx);
            if (checkFollower[0] == undefined){
                return res.json({success: true, code: 3101, message: "팔로잉이 존재하지 않습니다."});
            }

            const followingRow = await userModel.selectFollowingList(userIdx);
            return res.json({success: true, code: 1000, message: "팔로잉 리스트 성공", result: followingRow[0]});
        } catch (err) {
            console.log(error);
            return res.status(4000).send(`Error: ${err.message}`);
        }
